...
def main():
	...
	output_file = open(sys.argv[5], 'w', newline='')
...
def build_output(gene_id, RP_values, position_value, number, frame, fout):
	if position_value/frame - number - 1 < 0:
		start_pos = 0
	else:
		start_pos = int(position_value/frame) - number - 1

	if position_value/frame + number > len(RP_values):
		stop_pos = len(RP_values)
	else:
		stop_pos = int(position_value/frame) + number
